...
describe('Create files and folders', () => {
  ...
  it('generate policy default', async () => {
    const mock = mocksFiles.policyDefault();
    await generatePolicy.new(mock.name, mock.options, mock.projectPath);
    expect(path.join(mock.projectPath, 'policies/', mock.name + '.policy.js')).to.be.a.file().to.have.extname('.js')
  });

  it('generate policy admin', async () => {
    const mock = mocksFiles.policyAdmin();
    await generatePolicy.new(mock.name, mock.options, mock.projectPath);
    expect(path.join(mock.projectPath, 'policies/', mock.name + '.policy.js')).to.be.a.file().to.have.extname('.js')
  });
});
