      ...
      if(params.modules) {
        for (let module of params.modules) {
          if(typeof module === 'object'){
            ...
            modules += ` const ${capitalize(key)} = ${module[key]} ${manipulateJS.breakLine}`;
            continue
          }
          modules += ` const ${capitalize(module)} = require('${module}'); ${manipulateJS.breakLine}`
        }
      }

      params.fileContent = `${modules} ${manipulateJS.breakLine} module.exports = ${params.fileContent.replace(/"|\\n|\\r/gm, '')}`;
    }
  }
