...
const generatePolicy = require('../generate/generate.policy');
...
exports.new = async (name) => {
  ...
  await generateModel.new('user', { properties: 'firstname:string,age:number', methods: 'comparePassword', hooks: 'pre.save.hashPassword,pre.findOneAndUpdate.hashPassword,pre.findOneAndUpdate.addDate' }, projectPath);
  ...
  await generatePolicy.new('default', {}, projectPath);
  await generatePolicy.new('admin', {}, projectPath);
  await generateService.new('hashPassword', { modules: 'bcrypt,config' }, projectPath);
